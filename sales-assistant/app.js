(function () {
  ...
  function init() {
    ...
    const first = searchKnowledge()[0];
    state.selectedId = first ? first.item.id : "";
    render();
  }
  ...
  function render() {
    ...
    if (!state.selectedId || !matches.some((match) => match.item.id === state.selectedId)) {
      state.selectedId = matches[0] ? matches[0].item.id : "";
    }
    renderResults(matches);
    renderAnswer();
    renderGuardrails(matches);
    renderAdmin();
  }
  ...
  function searchKnowledge() {
    ...
    return KNOWLEDGE_BASE.map((item) => {
      ...
      if (!q) score += 1;
      if (q && haystack.includes(q)) score += 18;
      terms.forEach((term) => {
        if (term && haystack.includes(term)) score += 8;
      });
      if (state.role && item.roles.includes(state.role)) score += 7;
      if (state.objection && item.objections.includes(state.objection)) score += 8;
      if (state.device && item.devices.includes(state.device)) score += 5;
      if (state.industry && item.industries.includes(state.industry)) score += 3;
      state.systems.forEach((system) => {
        if (haystack.includes(normalize(system))) score += 4;
      });

      return { item, score };
    })
      .filter((match) => match.score > 0)
      .sort((a, b) => b.score - a.score || a.item.id.localeCompare(b.item.id, "zh-CN"));
  }

  function renderResults(matches = searchKnowledge()) {
  ...
  function renderAnswer() {
    const item = KNOWLEDGE_BASE.find((entry) => entry.id === state.selectedId);
    if (!item) {
    ...
    const composed = composeAnswer(item);
    els.emptyAnswer.hidden = true;
    els.answerCard.hidden = false;
    els.answerCategory.textContent = item.category;
    els.answerQuestion.textContent = item.question;
    ...
    els.sourceMeta.textContent = `来源：${item.source} / ${item.version} / ${item.updatedAt}`;

    const notice = getSensitiveNotice(item);
    els.sensitiveNotice.hidden = !notice;
    els.sensitiveNotice.textContent = notice;
  }

  function composeAnswer(item) {
    const role = state.role || firstOrDefault(item.roles, "客户");
    const industry = state.industry || "客户现场";
    const device = state.device || firstOrDefault(item.devices, "关键设备");
    const systemsText = state.systems.length ? `，并结合客户已有的${state.systems.join("、")}` : "";
    const lead = `如果对方是${role}，可以先把重点落在${device}的真实运行状态上`;
    const proofText = item.proof.slice(0, 3).join("；");
    const talkTrack = `${lead}。基于已审核资料，我们可以这样沟通：${item.core}${systemsText}。具体来看，${proofText}。建议先用小范围试点或现场台账验证，再进入报价、实施和验收细节。`;
    ...
    return {
      oneLine: item.core,
      talkTrack,
      followUps: unique([...item.followUps, ...contextFollowUps]).slice(0, 5),
      shareSummary: `您好，针对${industry}${device}场景，${item.share}涉及价格、证书、案例或合同承诺时，我们会提供正式审核资料。`
    };
  }
  ...
  function renderAdmin() {
    ...
    els.metricKnowledge.textContent = KNOWLEDGE_BASE.length;
